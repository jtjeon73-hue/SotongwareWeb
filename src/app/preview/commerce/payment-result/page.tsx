...
function ResultInner() {
  ...
  return (
    <PreviewShell title="결제 결과" subtitle="성공 · 실패 · 취소 예시" backHref={`${PREVIEW_BASE}/checkout?sku=${sku}`}>
      <div className={`mx-auto max-w-lg rounded-3xl border p-8 ${view.tone}`}>
        ...
        <div className="mt-8 flex flex-col gap-3">
          {status === "success" ? (
            <PreviewCta href={`${PREVIEW_BASE}/library?member=${kind === "subscription" ? "basic_active" : "free"}`}>
              내 자료실로 이동
            </PreviewCta>
          ) : (
